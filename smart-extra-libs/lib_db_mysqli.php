...
$configs['mysqli']['type'] 			= 'mysql'; 									// mysql / mariadb / percona
$configs['mysqli']['server-host'] 	= 'localhost';								// database host (default is 127.0.0.1)
		...
		if(!defined('SMART_FRAMEWORK_DB_LINK_MySQL')) { // MySQL default connection constant to avoid re-connection which can break transactions
			...
			if(array_key_exists((string)$the_conn_key, (array)SmartFrameworkRegistry::$Connections['mysqli'])) { // if the connection was made before using the SmartMysqliExtDb
				...
				if(is_object($y_connection)) {
					...
					if((string)$y_connection->thread_id != '') {
						//--
						define('SMART_FRAMEWORK_DB_VERSION_MySQL', self::check_server_version($y_connection));
						//--
					} //end if
					//--
				} //end if
 ...
 *
 */
public function escape_str($y_string) {
	//--
	return SmartMysqliDb::escape_str($y_string, $this->connection);
	//--
} //END FUNCTION
...
public function check_server_version() {
	//--
	return SmartMysqliDb::check_server_version($this->connection);
	//--
} //END FUNCTION
