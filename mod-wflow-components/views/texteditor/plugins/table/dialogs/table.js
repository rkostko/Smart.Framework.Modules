			...
			onOk: function() {
				...
				if ( data.info ) {
					...
					if ( !table.$.tHead && ( headers == 'row' || headers == 'both' ) ) {
						//var thead = new CKEDITOR.dom.element( table.$.createTHead() ); // unixman: #fix from v.4.5.8
						var thead = table.findOne( 'thead' ); // unixman: #fix from v.4.5.8
						tbody = table.getElementsByTag( 'tbody' ).getItem( 0 );
						var theRow = tbody.getElementsByTag( 'tr' ).getItem( 0 );
						//-- unixman: #fix from v.4.5.8
						if ( !thead ) {
