	...
	function initPasteClipboard( editor ) {
		...
		function createPasteCmd() {
			return {
				...
				exec: function( editor, data ) {
					var cmd = this; // unixman: #fix from v.4.5.9
					var fire = function( data, withBeforePaste ) {
						...
						//}, // unixman: #fix from v.4.5.9
						//cmd = this; // unixman: #fix from v.4.5.9
					}; // unixman: #fix from v.4.5.9
		...
		function getClipboardDataByPastebin( evt, callback ) {
			...
			// resize handler, pull it off the screen instead.
				...
				//pastebin.setStyle( editor.config.contentsLangDirection == 'ltr' ? 'left' : 'right', '-1000px' ); // unixman: #fix from v.4.5.9
				pastebin.setStyle( editor.config.contentsLangDirection == 'ltr' ? 'left' : 'right', '-10000px' ); // unixman: #fix from v.4.5.9
			}
	...
	CKEDITOR.plugins.clipboard = {
		...
		initPasteDataTransfer: function( evt, sourceEditor ) {
			if ( !this.isCustomCopyCutSupported ) {
				//return new this.dataTransfer( null, sourceEditor ); // unixman: #fix from v.4.5.11
				//-- unixman: #fix from v.4.5.11
	...
	CKEDITOR.plugins.clipboard.dataTransfer.prototype = {
		...
		cacheData: function() {
			...
			if ( ( this.$ && this.$.files ) || file ) {
				...
				// Edge have empty files property with no length (#13755). // unixman: #fix from v.4.5.11
				if ( this.$.files && this.$.files.length ) { // unixman: #fix from v.4.5.11
