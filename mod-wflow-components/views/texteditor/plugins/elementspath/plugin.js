...
( function() {
	...
	function initElementsPath( editor, bottomSpaceData ) {
		...
		function onClick( elementIndex ) {
			...
			var selection; // unixman: #fix from v.4.5.11
			if ( element.equals( editor.editable() ) || element.getAttribute( 'contenteditable' ) == 'true' ) {
				var range = editor.createRange();
				range.selectNodeContents( element );
				//range.select(); // unixman: #fix from v.4.5.11
				selection = range.select(); // unixman: #fix from v.4.5.11
			} else {
				//editor.getSelection().selectElement( element ); // unixman: #fix from v.4.5.11
				selection = editor.getSelection(); // unixman: #fix from v.4.5.11
				selection.selectElement( element ); // unixman: #fix from v.4.5.11
			}
