...
//	}
	...
	if(this.xmlInitLoadData) {
		this.io.fromXMLText(this.xmlInitLoadData);
...
SQL.Designer.prototype.getCookie = function() {
	...
	for (var i=0;i<parts.length;i++) {
		var part = parts[i];
		var r = part.match(/smartdbmodelerjs=({.*?})/);
		if (r) { obj = eval("("+r[1]+")"); }
	}
	return obj;
}
...
SQL.Designer.prototype.getOption = function(name) {
	...
	switch (name) {
		case "readonly":
			return CONFIG.IS_READONLY;
			break;
		case "savedatatypes":
			return CONFIG.XML_SAVE_DATATYPES;
			break;
		case "locale":
		...
		case "staticpath":
			...
			break;
		case "xhrsavefunction":
			return CONFIG.XHR_SAVE_FUNCTION || null;
			break;
		...
		case "hide":
			...
			break;
		case "vector":
			return true; // if false will not draw smooth connectors
		default:
			console.error('wwwsqldesigner: Invalid Option: ' + name);
			return null;
	}
}
...
SQL.Designer.prototype.toXML = function() {
	...
	xml += '<!-- SQL XML created by SmartFramework.Modules.WFlow.DbModeler -->\n';
//	xml += '<!-- Active URL: ' + location.href + ' -->\n';
//	xml += '<sql>\n';
	xml += '<sql db="' + SQL.escape(this.getOption("db")) + '">\n';

	/* serialize datatypes */
	if(this.getOption("savedatatypes")) {
