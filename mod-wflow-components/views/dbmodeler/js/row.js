...
SQL.Row.prototype.buildEdit = function() {
	...
	var isModeReadOnly = this.owner.owner.getOption("readonly");
	...
	if(isModeReadOnly) {
		this.dom.name.disabled = 'disabled';
	} //end if
	...
	if(isModeReadOnly) {
		this.dom.type.disabled = 'disabled';
	} //end if
	...
	if(isModeReadOnly) {
		this.dom.size.disabled = 'disabled';
	} //end if
	...
	if(isModeReadOnly) {
		this.dom.def.disabled = 'disabled';
	} //end if
	...
	if(isModeReadOnly) {
		this.dom.nll.disabled = 'disabled';
	} //end if
...
SQL.Row.prototype.redraw = function() {
	...
	OZ.DOM.removeClass(this.dom.title, "unique"); // unixman
	...
	//-- unixman
	if(this.isUnique()) {
		OZ.DOM.addClass(this.dom.title, "unique");
	}
	//-- #
	if(this.isKey()) {
...
SQL.Row.prototype.toXML = function() {
	...
	var t = this.getTitle(); // .replace(/"/g,"&quot;"); # fixed by unixman with SQL.escape() below
	var nn = (this.data.nll ? "1" : "0");
	var ai = (this.data.ai ? "1" : "0");
	xml += '<row name="'+SQL.escape(t)+'" null="'+nn+'" autoincrement="'+ai+'">\n';
...
//			d = q+d+q;
		} else { // unixman: fix default value quote
			d = d.trim();
			if((d.substr(0,1) == "'") && (d.substr(-1,1) == "'")) {
				d = d.substr(1);
...
//console.log(r);
	...
	if (d.length && d[0].firstChild) {
		...
		if (q) {
			...
			if(r) {
			//	obj.def = r[1];
				obj.def = "'" + r[1] + "'"; // unixman: fix default value quote
			}
		}
	}
