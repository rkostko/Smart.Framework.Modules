...
var qMockupEditor = new function() { // START CLASS
	...
	this.saveDocumentCode = function() {
		if(typeof qMockupEditorSave === 'function') {
			var documentstring = $('#canvas').html(); // unixman (save just canvas contents ...)
			qMockupEditorSave(documentstring);
		} //end if
	} //END FUNCTION

	this.loadDocumentCode = function(data) {
		$('#canvas').html(data); // unixman (load canvas contents ...)
	} //END FUNCTION
