...
// (c) 2012 https://sourceforge.net/p/bcmath-js/code/11/tree/src/
// (c) 2007-2018 Kevin van Zonneveld (http://kvz.io) and Contributors (http://phpjs.org/authors)

//=======================================
// CLASS :: BC Math (Fields)
//=======================================

var BC_Math = new function() { // START CLASS :: v.180305


	this.bcscale = function(scale) {
	...
	this.bccomp = function(left_operand, right_operand, scale) {
	...
	//	var libbcmath = this._phpjs_shared_bc();
		...
		return libbcmath.bc_compare(first, second, scale); // int -1, 0, 1

	} //END FUNCTION


	this.bcadd = function(left_operand, right_operand, scale) {
	...
	this.bcsub = function(left_operand, right_operand, scale) {
	...
	this.bcmul = function(left_operand, right_operand, scale) {
	...
	this.bcdiv = function(left_operand, right_operand, scale) {
	...
	//	var libbcmath = this._phpjs_shared_bc();
		...
		if(result === -1) {
			throw new Error(11, '(BC) Division by zero'); // error
		} //end if
		if(result.n_scale > scale) {
			result.n_scale = scale;
		} //end if
	...
	// to be tested ...
	this.bcpow = function(left, right, scale) {
		// http://kevin.vanzonneveld.net + original by: Robin Speekenbrink (http://www.kingsquare.nl/))
	...
	// to be tested ...
	this.bcsqrt = function(operand, scale) {
		// http://kevin.vanzonneveld.net + original by: Robin Speekenbrink (http://www.kingsquare.nl/))
	...
	this.bcround = function(val, precision) {
	...
	//	var libbcmath = this._phpjs_shared_bc();
		...
		if(precision >= temp.n_scale) {
			while (temp.n_scale < precision) { // nothing to round, just add the zeros.
		...
		if(digit >= 5) {
			right_operand.n_value[right_operand.n_len + right_operand.n_scale - 1] = 1; // round away from zero by adding 1 (or -1) at the "precision".. ie 1.44999 @ 3dp = (1.44999 + 0.001).toString().substr(0,5)
			if(temp.n_sign == libbcmath.MINUS) {
				right_operand.n_sign = libbcmath.MINUS; // round down
			} //end if
			result = libbcmath.bc_add(temp, right_operand, precision);
		} else {
			result = temp; // leave-as-is.. just truncate it.
		} //end if else
	...
	var libbcmath = {
			...
			if(!zero) { /* Normalize */
				...
				while(qdig <= len1 + scale - len2) { /* Calculate the quotient digit guess. */
					...
					if(qguess !== 0) {
						...
						// @CHECK: Does a negative pointer return null ? ptr2 can be < 0 here as ptr1 = len2, thus count < len2+1 will always fail ?
		...
		*/
		...
		_bc_do_sub: function(n1, n2, scale_min) {
			...
			/* Not needed?
			if(scale_min > diff_scale) { // Zero extra digits made by scale_min.
