...
function _classCallCheck(instance, Constructor) {
	...
	var Codebird = (function() {
		...
		_createClass(Codebird, [
			{
				key: "sf_auth",
				value: function sf_auth(xml, url) {
					// unixman
					var ua = "";
					try {
		...
		*/
			...
			},
			{
				...
				value: function _getXmlRequestObject() {
					...
					if (
						...
						(typeof Ti === "undefined" ? "undefined" : _typeof(Ti)) ===
							"object" &&
						Ti &&
			...
			},
			{
				...
				value: function _detectMethod(method, params) {
					...
					for (var _httpmethod in apimethods) {
						if (
							apimethods.hasOwnProperty(_httpmethod) &&
							apimethods[_httpmethod].indexOf(method) > -1
						) {
							return _httpmethod;
						}
					}
			...
			},
			{
				...
				value: function oauth_authenticate() {
					...
					if (this._oauth_token === null) {
						var error =
							"To get the " + type + " URL, the OAuth token must be set.";
						if (dfd) {
			...
			},
			{
				...
				value: function _callApi(httpmethod, method) {
					...
					if (httpmethod === "GET") {
						...
						this.sf_auth(xml, url_with_params); // unixman
					} else {
						if (multipart) {
							if (!app_only_auth) {
