...
final class Codebird {
	...
	protected function _validateSslCertificate($validation_result)
	{
		if (in_array(
				$validation_result,
				[
					(int) self::$CONST_CURLXE_SSL_CERTPROBLEM,
					(int) self::$CONST_CURLXE_SSL_CACERT_ISSUE,
					(int) self::$CONST_CURLXE_SSL_CACERT_BADFILE,
					(int) self::$CONST_CURLXE_SSL_CRL_BADFILE,
					(int) self::$CONST_CURLXE_SSL_ISSUER_ERROR
				]
			)
		) {
			throw new \Exception(
