/*
 * Flexigrid for jQuery -  v1.1.unixman.2
 * Fixes:
 * 		# removed duplicate browser detection)
 * 		# renamed get/post url params
 *
	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				var param = [{
					...
					name: 'flexigrid_resperpage',
					value: p.rp
				}, {
					name: 'flexigrid_sortfld',
					value: p.sortname
				}, {
					name: 'flexigrid_sortdir',
					value: p.sortorder
				}, {
					name: 'flexigrid_search',
					value: p.query
				}, {
					name: 'flexigrid_srcby',
					value: p.qtype
				}];
				if (p.params.length) {
		...
		if (p.usepager) {
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend("<div class='pGroup'><select name='flexigrid_resperpage'>" + opt + "</select></div> <div class='btnseparator'></div>");
